...
public class ProductsDao {
    ...
    // para facilitar, copiar codigo de outro xxxDao para instanciar base de datos
    ...
    public boolean registerProductQuery(Products product) {
        ...
        // acesar base de datos, usar try catch
        try {
    ...
    // Listar producto
    // dessa vez vai ser diferente, vai criar uma sql (view?) pra acesar via este metodo
    //
    public List listProductsQuery(String value) {
        List<Products> list_products = new ArrayList();
        String query = "SELECT pro.*, ca.name AS caregory_name FROM products pro, categories ca WHERE pro.category_id = ca.id";
        String query_search_product = "SELECT pro.*, ca.name AS category_name FROM produtcts pro INNER JOIN categories ca "
        ...
        try {
            ...
            while (rs.next()) {
                ...
                product.setProduct_quantity(rs.getInt("produtct_quantity"));
    ...
    public boolean deleteProductQuery(int id) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "No puede eliminar producto que tenga relación con otra tabla");
            return false;
        }
    }
    ...
    public boolean updateStockQuery(int amount, int product_id){
        String query = "UPDATE products SET producty_quantity=? WHERE id=?";
        try{
            conn = cn.getConnection();
            pst = conn.prepareStatement(query);
            pst.setInt(1, amount);
