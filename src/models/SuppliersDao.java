...
public class SuppliersDao {
    ...
    // instanaciar a conexion
    ConnectionMySQL cn = new ConnectionMySQL();
    Connection conn;
    PreparedStatement pst;
    ResultSet rs;

    // registrar proveedor (supplier)
    public boolean registerSupplierQuery(Suppliers supplier) {
        String query = "INSERT INTO suppliers (name, description, address, telephone, "
                + "email, city, created, updated) VALUES (?,?,?,?,?,?,?,?)";
        Timestamp datetime = new Timestamp(new Date().getTime());

        // al acceder a base de datos usar try catch
        try {
